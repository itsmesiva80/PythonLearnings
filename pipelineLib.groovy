...
def rcPatch(String commithash, String rcname, String projectName){

    git credentialsId: 'github-com-flaconi', url: 'git@github.com:Flaconi/' + projectName + '.git'

    output = shr("git branch --contains " + commithash)
    if (output == "") {
        error 'COMMIT_HASH for the patch does not exist in the master branch'
    }
    //clean and checkout
    sh 'git checkout tags/'+ rcname
    sh 'git config user.email "jenkins@flaconi.de"'
    sh 'git config user.name "Jenkins CI"'
    sh 'git checkout -b ' + rcname + '-patches'
    //cherrypick
    sh 'git cherry-pick ' + commithash
    //tag and push
    sh 'git tag --force ' + rcname
    sh 'git push --force origin tags/' + rcname
}
return this
